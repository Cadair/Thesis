...
There is currently no understood mechanism which transports this energy, from the photosphere, through the transition region and into the corona. %TODO: Check the 100 w/m2 number, then reference it.
...
vc = vso.VSOClient()
res = vc.query(vso.attrs.Time('2015/8/15', parse_time('now'), parse_time('now')), vso.attrs.Instrument('AIA'), vso.attrs.Wave(17.1*u.nm, 17.1*u.nm))
filepath = vc.get(res).wait()[0]
