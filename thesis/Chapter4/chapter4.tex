...
%In \cref{fig:All-TD-wave-30} it can be seen that the torsional driver excites perturbations in each velocity component, \textit{e.g.} $V_\phi$,  $V_\perp$, $V_\parallel$, as would be expected for a driver that is not exactly a linear eigenmode of the system. 
...
First, we take the case of the horizontal driver, \cref{fig:All-TD-wave-30:horiz}, in the most detail. 
...
for j, (driver, exp_fac, caption) in enumerate(zip(drivers, exp_facs, captions)):
    ...
    plt.subplots_adjust(left=0.25,right=0.90,top=0.95,bottom=0.17)
...
for j, (driver, exp_fac, caption) in enumerate(zip(drivers, exp_facs, captions)):
    ...
    colorbars[0].ax.set_ylabel(r"F$_\parallel / $F$^2$")
    colorbars[1].ax.set_ylabel(r"F$_\perp^2 / $F$^2$")
    colorbars[2].ax.set_ylabel(r"F$_\phi^2 / $F$^2$")
