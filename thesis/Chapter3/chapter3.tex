...
The numerical simulations performed for the MHD wave mode analysis in this thesis, all make use of the ability of SAC (\cref{sec:SAC}) to solve for perturbations on top of a static MHD equilibria.
...
\cite{utz2013} performed a similar study looking at the average magnetic field strength of MBPs using the Hinode SOT instrument.
...
This algorithm makes use of the Visualisation Tool Kit (VTK\footnote{VTK 6.1 (\url{www.vtk.org})}) and the MayaVi package \citep{Ramachandran2011} to provide a high level Python interface to VTK.
