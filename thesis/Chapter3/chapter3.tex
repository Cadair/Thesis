...
The vertical profile of the magnetic field, along the axis of the flux tube, is given in \cref{eq:BO}, and it is a summation of two Gaussians, one for controlling the profile in the low atmosphere and one for controlling the profile in the upper atmosphere.
...
f \,   &= r \BO,  \label{eq:f}
\\
\GO \, &= \exp\left(-\frac{f^2}{f_0^2}\right),  \label{eq:G}
\\
...
Similarly to \cite{gent2014} a uniform background field ($\bc$) is also added to the model, this background field provides for a more realistic atmosphere, instead of an atmosphere where there is only one magnetic flux tube present.
The background field is designed to model the small-scale and distant influence of other magnetic structures in the solar atmosphere.
...
\begin{table}
    ...
    \begin{tabular}{|c|c|}
        ...
        \hline $\bc$ & \py[chapter3]|model_pars['pBplus'].to(u.mT)| \\ 
...
%TODO: Magnetic Field Properties:
%* footpoint bfield
%* Expansion rates
%* equation for field construction.
%* equation for hydrostatic equlibrium.

The result of the flux tube construction in \cite{gent2013, gent2014} is a 2D Gaussian profile for the cross-sectional magnetic field, with a full width at half maximum of $\approx$ \py[chapter3]|np.round(fwhm, -1)|, and a full width at a 10th of maximum of \py[chapter3]|np.round(fwtm, -1)| at the photosphere, corresponding to the lower bound of the observations in \cite{utz2013}.
...
m3d = ch3.save_figure('domain-3d', fig, fext='.png', size=(1200,900),
                      ...
                      focalpoint=[ 75.,  16.,  30.])
m3d.caption=r"A 3D view of the background atmosphere. Magnetic field lines are shown in green (log scale), density (log scale) in purple and thermal pressure (log scale) in red-orange. The full width at half, tenth and 100th of maximum are shown in red contours at the base of domain."
...
However, the challenge is decomposing the velocity vector calculated by SAC in the reference frame of the simulation domain ($Vx$, $Vy$, $Vz$) into the frame of the magnetic field \textit{i.e.} a velocity parallel to the magnetic field, $V_\parallel$, normal to the magnetic field, $V_\perp$, and a component perpendicular to the magnetic field in the other plane $V_\phi$, in which the waves are mathematically described.
...
The chosen solution to finding a perpendicular vector in three dimensions is to consider the concept of flux tubes.
...
The process of computing these flux surfaces is illustrated in \cref{fig:circ-seeds,fig:fieldlines,fig:flux-surface,fig:zoomed-surface,fig:zoomed-glyphs} and described in the respective captions.
