...
This approach enabled the code to solve for small perturbations, while the background conditions vary by orders of magnitude through the solar atmosphere.
...
The SAC code was developed to simulate MHD waves as solutions to the ideal MHD equations given in \cref{eq:imhd_mass,eq:imhd_induction} to achieve this while maintaining the ability to simulate a realistic solar atmosphere the equations were re-written into a form where they have separate background and perturbation components allowing a stable stratified background model to be setup and then preserved against the effects of gravity.

The equations solved by SAC are a rearranged version of the ideal MHD equations and a given below \citep[taken from][]{Shelyag2008},
\begin{equation}
