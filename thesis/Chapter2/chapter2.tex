...
This process makes use of the Visualisation Tool Kit (VTK\footnote{Visualisation ToolKit 5.10 (\url{www.vtk.org})}) and the MayaVi package \cite{Ramachandran2011} to provide a high level Python interface to VTK.
...
As the magnetic field is axisymmetric the flux surfaces chosen are also axisymmetric, therefore, the first stage in the construction is to define a ring of seed points around the centre of the domain, shown in \cref{fig:circ_seeds}.
...
caption=r"Outline of the numerical domain showing the seed points at a radius of $936$ km from the centre of the domain.")|

The next stage is to trace field lines down through the domain from these seeds.
The lines, traced using VTKs streamline algorithm are shown in \cref{fig:fieldlines}.
...
%\begin{pycode}[chapter2]
%fieldlines, surface = ttf.create_flux_surface(bfield, seeds)
...
%mayavi_savefig('zoomed_surface', scene, azimuth = 90, elevation = 75, distance=80, focalpoint=[63, 120, 110])
%\end{pycode}
